...
import { SearchInfo,Message } from '@/interfaces';
  ...
  const handleSubmit = async (e:any) => {
    ...
    if (currentMessage.trim()) {
      ...
      try {
        ...
        let url = `${baseUrl}/chat-stream/${encodeURIComponent(userInput)}`;
        if (checkpointId) {
          url += `?checkpoint_id=${encodeURIComponent(checkpointId)}`;
        }

        // Connect to SSE endpoint using EventSource(provide by browser)
        // This will stream the response from the server
